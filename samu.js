...
async function starts() {
	...
	nzwa.on('qr', () => {
		console.log(color('[','white'), color('!','red'), color(']','white'), color('Escanea el código'))
	})

	fs.existsSync('./Samu330.json') && nzwa.loadAuthInfo('./Samu330.json')
	nzwa.on('connecting', () => {
		start('2', 'Conectando...')
	})
	nzwa.on('open', () => {
		success('2', '[BOT] NyanBot listo!')
	})
	await nzwa.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./Samu330.json', JSON.stringify(nzwa.base64EncodedAuthInfo(), null, '\t'))

        console.log('=> NyanBot')
        lolcatjs.fromString('Contactame para nuevas actualizaciones')
		...
		try {
			...
			const isCmd = body.startsWith(prefix)
                        ...
                        mess = {
							wait: '⌛ 𝐄𝐍 𝐏𝐑𝐎𝐂𝐄𝐒𝐎 ⌛',
							success: '✔️ 𝙎𝙐𝙎𝙎𝙀𝙎 ✔️',
							levelon: '❬ ✔ ❭ *𝙉𝙞𝙫𝙚𝙡 𝙝𝙖𝙗𝙞𝙡𝙞𝙩𝙖𝙙𝙤*',
							leveloff: ' ❬ X ❭  *𝙉𝙞𝙫𝙚𝙡 𝙙𝙚𝙨𝙝𝙖𝙗𝙞𝙡𝙞𝙩𝙖𝙙𝙤*',
							levelnoton: '❬ X ❭ *𝙔𝙖 𝙖 𝙚𝙨𝙩𝙖𝙙𝙤 𝙖𝙘𝙩𝙞𝙫𝙤 𝙖𝙣𝙩𝙚𝙨*',
							levelnol: '*𝐍𝐈𝐕𝐄𝐋* 0',
							error: {
							...
							only: {
								...
								daftarB: `──「 NO ESTÁS REGISTRADO 」──\n Usa el comando: ${prefix}reg para registrarte`,
                                }
                        }
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '14694222222@s.whatsapp.net'
 	   	...
 	   	if (isGroup && isBadWord) {
                        if (bad.includes(messagesC)) {
                                if (!isGroupAdmins) {
                                        ...
                                        }).catch(() => nzwa.sendMessage(from, `Te salvaste pq no soy admin si no te saco a patadas!`, text , {quoted : mek}))
                                } else {
               ...
               break
				...
				case 'rebaixar':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Un admin menos @${mentioned[0].split('@')[0]}`)
						nzwa.groupDemoteAdmin(from, mentioned)
					}
					break
                                case 'randomhentai':
              ...
              await nzwa.sendMessage(from, options, text)
               break
				...
				case 'snowwrite':
					...
					var gbl7 = gh.split("|")[0];
					var gbl8 = gh.split("|")[1];
					if (args.length < 1) return reply(`Usa el comando correctamente ${prefix}snowwrite texto1|texto2, Ejemplo ${prefix}snowwrite Bot|𝐓𝐨𝐱𝐢𝐜`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(mess.wait)
				...
				case 'marvellogo':
					...
					if (args.length < 1) return reply(`Usa el comando correctamente ${prefix}marvellogo texto, Ejemplo  ${prefix}marvellogo 𝐓𝐨𝐱𝐢𝐜 bot`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(mess.wait)
	          ...
	          if (!isRegister) return reply(mess.only.daftarB)
               ...
               if (anu.error) return reply(anu.error)
				...
				case 'logowolf':
					...
					if (args.length < 1) return reply(`Y el texto? Ejemplo ${prefix}logowolf 𝐓𝐨𝐱𝐢𝐜|Bot`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(mess.wait)
				...
				case 'logowolf2':
					...
					var gbl1 = gh.split("|")[0];
					var gbl2 = gh.split("|")[1];
					if (args.length < 1) return reply('Y el texto? Ejemplo ${prefix}phlogo 𝐓𝐨𝐱𝐢𝐜|Bot')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(mess.wait)
            ...
            break
				        ...
				        uptime = process.uptime()
                                ...
                                case 'neonlogo':
                                        var gh = body.slice(9)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`Y el texto?\nEjemplo: ${prefix}neonlogo 𝐓𝐨𝐱𝐢𝐜Bot`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
                                ...
                                case 'neonlogo2':
                                        ...
                                        teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`Y el texto?\nEjemplo: ${prefix}neonlogo2 𝐓𝐨𝐱𝐢𝐜Bot`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
                                ...
                                case 'lionlogo':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        var teks2 = gh.split("|")[1];
                                        if (args.length < 1) return reply(`Y el texto?\nEjemplo: ${prefix}lionlogo 𝐓𝐨𝐱𝐢𝐜|Bot`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
                                ...
                                case 'jokerlogo':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`Y el texto?\nEjemplo: ${prefix}jokerlogo 𝐓𝐨𝐱𝐢𝐜Bot`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
			       ...
			       reply(anu.result.jadwalTV)
					break*/
                                ...
                                case 'burnpaper':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`Y el texto?\nEjemplo: ${prefix}burnpaper 𝐓𝐨𝐱𝐢𝐜Bot`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
									...
									case 'qowheart':
											var gh = body.slice(9)
											var teks1 = gh.split("|")[0];
											if (args.length < 1) return reply(`Y el texto?\nEjemplo: ${prefix}qowheart 𝐓𝐨𝐱𝐢𝐜Bot`)
											if (!isRegister) return reply(mess.only.daftarB)
											if (isLimit(sender)) return reply(ind.limitend(pusname))
											reply(mess.wait)
									...
									case 'mutgrass':
											var gh = body.slice(9)
											var teks1 = gh.split("|")[0];
											if (args.length < 1) return reply(`Y el texto?\nEjemplo: ${prefix}mutgrass 𝐓𝐨𝐱𝐢𝐜Bot`)
											if (!isRegister) return reply(mess.only.daftarB)
											if (isLimit(sender)) return reply(ind.limitend(pusname))
											reply(mess.wait)
									...
									case 'woodenboards':
											...
											var teks1 = gh.split("|")[0];
											if (args.length < 1) return reply(`Y el texto?\nEjemplo: ${prefix}woodenboards 𝐓𝐨𝐱𝐢𝐜Bot`)
											if (!isRegister) return reply(mess.only.daftarB)
											if (isLimit(sender)) return reply(ind.limitend(pusname))
											reply(mess.wait)
									...
									case 'wolfmetal':
											var gh = body.slice(10)
											var teks1 = gh.split("|")[0];
											if (args.length < 1) return reply(`Y el texto?\nEjemplo: ${prefix}wolfmetal 𝐓𝐨𝐱𝐢𝐜Bot`)
											if (!isRegister) return reply(mess.only.daftarB)
											if (isLimit(sender)) return reply(ind.limitend(pusname))
											reply(mess.wait)
					...
					break
                                ...
                                case 'r':
                                    if (isRegister) return  reply(ind.rediregis())
                                        ...
                                        if (namaUser.length >= 30) return reply(`¿Por qué tu nombre es tan largo? ¿Es un nombre o un tren-_-?`)
