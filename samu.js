...
async function starts() {
	...
	fs.existsSync('./Samu.json') && Lxa.loadAuthInfo('./Samu.json')
	Lxa.on('connecting', () => {
		start('2', 'Connecting...')
	})
	Lxa.on('open', () => {
	...
	await Lxa.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./Samu.json', JSON.stringify(Lxa.base64EncodedAuthInfo(), null, '\t'))
...
if (getmemex <= memberlimit) {
  Lxa.sendMessage(from, `Los miembros de este grupo son insuficientes, se nececitan: ${memberlimit}, para usar este bot`, text)
