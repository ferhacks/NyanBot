...
//*********Balasan bot
			...
			const sendMess = (hehe, teks) => {
				samu330.sendMessage(hehe, teks, text, {quoted: mek, contextInfo: {"forwardingScore": 9999, "isForwarded": true}
})
			}
...
const isQuotedSticker = type === 'extendedTextMessage' && content.includes('stickerMessage')
		...
		/*if (isGroup && isAntiMedia) {
			if (isMedia && isQuotedImage) {                                                                        
		if (!isGroupAdmins) {                                                                                             samu330.updatePresence(from, Presence.composing)

                    	 reply(`Media detectada, ${sender.split("@")[0]}`)
			samu330.groupRemove(from, `${sender.split("@")[0]}@s.whatsapp.net`)
...
samu330.sendMessage(from, buf, audio, {
  mimetype: 'audio/mp4', quoted: mek, duration :-999999999999999, ptt: true
})
}
...
${bodyM}${prefix}tomp3 (video a mp3)
${bodyM}${prefix}pubg 
${bodyM}${prefix}sombra
${bodyM}
...
//********* temp
			...
			case 'listvid':
				...
				/*case 'antimedia':
                                        if (!isGroup) return reply(mess.only.group)
                                        if (!isGroupAdmins) return reply(mess.only.admin)                                                       //  if (args.length < 1) return reply('Escribe *1* para activar')
                                        if (args[0] === '1') {                                                                                            if (isAntiMedia) return reply('*Ya está activo*')                                                           antimedia.push(from)                                                                                        fs.writeFileSync('./data/antimedia.json', JSON.stringify(antimedia))                                                                                                                                                    reply(`*[ Activado ]*\npersona que envie *fotos* o *videos*, sera eliminada.`)
                                        } else if (args[0] === '0') {
                                                antimedia.splice(from, 1)                                                                                   fs.writeFileSync('./data/antimedia.json', JSON.stringify(badword))                                                                                                                                                    reply(`Desactivado`)
                                        } else {                                                                                                          reply('1 para activar, 0 para desactivar')
                                        }
                                        break*/
