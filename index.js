		...
		try {
			...
			switch(command) {
				...
				case 'info':
					...
					teks = `*Nombre del bot* : ${me.name}\n*Numero del bot Bot* : @${me.jid.split('@')[0]}\n*Prefix* : ${prefix}\n*Total Block Contact* : ${blocked.length}\n*Tiempo de actividad del bot* : ${kyun(uptime)}`
				...
				case 'tp':
					if (args.length < 1) {
						return reply('Elige un tema, 1 - 162')
				...
				case 'sticker':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								console.log('Finish')
								exec(`webpmux -set exif ${addMetadata('Samu330:D', authorname)} ${ran} -o ${ran}`, async (error) => {
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								console.log('Finish')
								exec(`webpmux -set exif ${addMetadata('SAMU330ByNyanBot', authorname)} ${ran} -o ${ran}`, async (error) => {
									if (error) return reply(mess.error.stick)
									client.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek})
									fs.unlinkSync(media)
									fs.unlinkSync(ran)
						...
						await removeBackgroundFromImageFile({path: media, apiKey: keyrmbg, size: 'auto', type: 'auto', ranp}).then(res => {
							...
							exec(`ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${ranw}`, (err) => {
								...
								exec(`webpmux -set exif ${addMetadata('Samu330NoBg', authorname)} ${ranw} -o ${ranw}`, async (error) => {
									if (error) return reply(mess.error.stick)
				...
				case 'gtts':
					...
					break
	case 'image':
            if (args.length == 0) return reply('Que quiere buscar!')
			...
			console.log(erest)
            await client.sendMessage(from, erest, image, {quoted: mek, caption: ':D'})
			.catch(() => {
                        reply(from, 'No se ha recibido ninguna imagen o el servidor está desconectado, inténtalo más tarde.')
            })
            break
				case 'meme':
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('Aquien quieres agregar, a un fantasma?')
					...
					try {
						...
						console.log('Error :', e)
						reply('No se pudo agregar el numero, tal vez porque es privado')
					}
					break
				case 'kick':
					if (!isGroup) return reply(mess.only.group)
